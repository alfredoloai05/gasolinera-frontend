...
function ClientesCrud() {
  ...
  const handleOpenEditModal = (cliente) => {
    setEditCliente({
      ...
      DespachoSiNo: cliente.DespachoSiNo,
      codigo: cliente.codigo,
      EntidadPublica: cliente.EntidadPublica,
      NumeroCuantia: cliente.NumeroCuantia,
      NumeroCuantiaChecked: cliente.NumeroCuantia ? true : false,
    });
  ...
  const handleSubmit = async (e) => {
    ...
    const clienteData = { ...newCliente };
    delete clienteData.NumeroCuantiaChecked;

    try {
      await axios.post(`${config.apiUrl}/cliente`, clienteData, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
      });
      fetchClientes();
      handleCloseAddModal();
      setNewCliente({
        ...
        EntidadPublica: false,
        NumeroCuantia: "",
        NumeroCuantiaChecked: false, // Reiniciar el estado del checkbox
      });
    } catch (error) {
      console.error("Error al crear cliente", error);
      if (error.response) {
        console.log("Error data:", error.response.data); // Muestra los detalles del error
      }
    }
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    // Crear una copia del cliente y eliminar campos innecesarios
    const clienteData = { ...editCliente };
    delete clienteData.NumeroCuantiaChecked; // Eliminar campo que no debe enviarse
    ...
    <div>
      ...
      <CrudTable
        columns={[
          ...
          {
            title: "Despacho Si/No",
            field: "DespachoSiNo",
            render: (rowData) => (rowData.DespachoSiNo ? "Sí" : "No"),
          },
          {
            title: "Código",
            field: "codigo",
          },
          {
            ...
            field: "EntidadPublica",
            render: (rowData) => (rowData.EntidadPublica ? "Sí" : "No"),
          },
          {
            title: "Cuantía",
            field: "NumeroCuantia",
        ...
        actions={[
          {
            icon: () => <LocalParkingIcon />, // Asegúrate de que esto sea un componente válido
      ...
      <Modal open={openAddModal} onClose={handleCloseAddModal} aria-labelledby="modal-modal-title">
        <Box sx={style} component="form" onSubmit={handleSubmit}>
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={newCliente.NumeroCuantiaChecked} // Nuevo estado para el checkbox
      ...
      <Modal open={openEditModal} onClose={handleCloseEditModal} aria-labelledby="modal-modal-title">
        <Box sx={style} component="form" onSubmit={handleEditSubmit}>
          ...
          <TextField
            ...
            name="NumeroCuantia"
            fullWidth
            value={editCliente.NumeroCuantia}
            onChange={handleEditChange}
            margin="normal"
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={editCliente.NumeroCuantiaChecked} // Utiliza el estado del checkbox
                onChange={(e) =>
                  setEditCliente({
                    ...editCliente,
                    NumeroCuantiaChecked: e.target.checked,
                    NumeroCuantia: e.target.checked ? editCliente.NumeroCuantia : "", // Limpia el campo "NumeroCuantia" si se desmarca
                  })
                }
                name="NumeroCuantiaChecked"
              />
